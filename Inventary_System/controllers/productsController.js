 ...
 *    the function retrieves the list of books from the database using the getBooks method from the data model,
 *    if the user is authenticated, it also retrieves the current user's information from the session and checks if the user has an administrator role to retrieve the list of users.
 ...
 *    If the data is valid, it calls the addBook method from the data model to add the new book to the database and
...
async function addBook(req, res) {
    try {
        ...
        await data.addBook(title, priceNum, author, category);
 ...
 *    If the IDs are valid, it calls the deleteBooks method from the data model to delete the specified books from the database(200 code status).
 * @param {*} req 
 * @param {*} res 
 * @returns 
 */
 ...
 *    If the data is valid, it calls the updateBook method from the data model to update the specified book in the database(200 code status).
 * @param {*} req 
 * @param {*} res 
 * @returns 
 */
async function updateBook(req,res){
    try{
        const { id, title, price, author, category, quantity } = req.body;
        if (!id || !title || !price) {
            return res.status(400).send("Missing Data");
        }

        await data.updateBook(id, title, price, author, category, quantity);
        res.sendStatus(200);
    }catch(error){
        console.error(error);
 ...
 *    If the data is valid, it calls the updateQty method from the data model to update the quantity of the specified book in the database(200 code status).
 * @param {*} req 
 * @param {*} res 
 * @returns 
 */
